...
import com.yxz.mySimpleMVC.config.ConfigGet;
...
public class DispatcherServlet extends HttpServlet {
	...
	static {
		urlMapping = new DefaultUrlMapping();
		urlMapping.init();
	}
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 if(handler != null) {
			 Class<?> controllerClass = handler.getHandlerClass();
			 ...
			 if(result instanceof JspView) { //jsp视图
				 ...
				 if(path != null && path.length() != 0) {
					 if(true) { //请求forward
						 ...
						 req.getRequestDispatcher(ConfigGet.getJspFilePath() + path).forward(req, resp);
					 }
